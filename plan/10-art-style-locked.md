...
## 4.10 ★ 古代典故故事重演模式(2026-07-07 用户锁定)
文案是**古代典故/成语故事**（如杯弓蛇影）时，故事段人物**就画古人**（束发髻/长袍/古代器物），4.7 不适用于此：
- **不用角色定妆参考、不保 IP 一致**：flux 只喂**风格锚图**借画风（模板 `image-story.tpl.md`，`beat.refMode="style"`），人物全靠 shot.content 文字描述；**同一典故人物的外观描述各拍逐字复用**减少漂移。
- **文案说几个人就画几个人**：非 IP 古人多人同框也走 flux（$0.04），nano 只留给 IP 多角色。
- **先理解典故再写提示词**：器物形制/比例要对——例：杯弓蛇影的"杯"是**汉代宽口浅漆耳杯（大酒器）**，弓的倒影是**横贯整杯的长蛇**，不是小蚯蚓。
- 回到**现代解释道理**的拍，按需用固定 IP（孩子/父母，定妆参考照旧）。

## 4.7 ★★ 死命令：人是"现代小孩穿汉服 + 戴古风小帽",不是古代人/古代场景(用户 2026-07-03 锁定,帽子 2026-07-05 加锁,**仅适用金句朗读方向**；典故故事重演走 4.10)
