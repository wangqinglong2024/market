...
- **不变量**：配音永远中文，但**多角色分工**（童声/成年男/成年女/开朗姐姐旁白，见 [[04-tts-captions]]）；字幕三行（拼音/中文/越南语意译）；竖屏 9:16；画风统一；人物按需出场；特效 ≤2/片、按需。
...
- `shots`：图数量与每图内容。**判定规则**——
  ...
  - 否则 → **1 图**。
  - 每个 shot 给一句 `content`：**只描述该句文字所指的画面**，不得脑补无关内容（画面必须忠于原文）。
...
### 第 5 步 · 配音 + 时间轴
- 每个 beat 的中文 → 火山 TTS，**按该拍 `voice` 角色**（童声/成年男/成年女/开朗姐姐旁白，见 [[04-tts-captions]]）→ `mp3` + `durationMs`。
