...
## 0. 输入 / 输出契约
- **输入**：一段中文旁白文案（纯文本，长短不限）。可选：`videoId`、`lang`（默认 `vi` 越南语）。
- **输出**：`public/videos/<videoId>/` 下的 `成片.mp4` + 全部中间产物（可追溯、可重渲染）。
- **不变量**：配音永远中文（火山·开朗姐姐）；字幕三行（拼音/中文/当地语）；竖屏 9:16；画风统一。
...
### 第 5 步 · 配音 + 时间轴
- 每个 beat 的中文 → 火山 TTS（开朗姐姐）→ `mp3` + `durationMs`。
