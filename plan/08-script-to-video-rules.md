...
- **不变量**：配音永远中文，**全片统一旁白「开朗姐姐」**（不做多角色分工，见 [[04-tts-captions]]）；字幕**一屏一句短句**——单行拼音 / 单行中文 / 单行越南语意译，**禁止多行**（2026-07-07 用户锁定）；竖屏 3:4（1080×1440，2026-07-07 改版）；画风统一；人物按需出场；特效 ≤2/片、按需；**纯文化普及、禁止任何营销/广告（下载 CTA、软件推广、"一起学中文"等一律不要）**。

---

## 1. 核心数据单位：beat（一拍）＝一句**短句**（★ 2026-07-07 用户改版：语音/字幕拆细，图按场景走）
**一个 beat = 一句短句**（单行放得下的一小句），它是「配音 ↔ 字幕」同步的最小绑定单位。一个 beat 同时拥有：
- 原文中文**一句短句**（单行）
- 该短句的配音音频（决定该 beat 时长）
- 该短句三语字幕（单行拼音 / 单行中文 / 单行越南语）+ 字级时间戳 `charTimings[]`（逐字跳字）

**画面与 beat 解绑（关键改动）**：图不再"一语音一图"，而是按**场景/语义整体**聚合——
- 多个连续 beat 可**共用同一张图**（`sceneId` 相同即共图）。例：一句古诗上句+下句 = 2 个 beat（分开读、分开上字幕），但整句诗共配 **1 张图**。
- 共图的相邻 beat 之间**不换图、不转场**，运镜跨 beat 连续（见 [[03-remotion-animation]]）。
- 单 beat 内多 shot（表现过程）机制保留，但仅在确实需要时用。

> 「文字 / 配音 / 字幕」仍从同一个 beat 派生、共享同一时间窗，天生同步；
> 画面以**场景**为单位铺在其覆盖的 beat 区间之下。**字幕短句才是用户注视的重点**。

---

## 2. 流水线七步（每步规则写死）

### 第 1 步 · 切句（normalize → beats）（★ 2026-07-07 改版：切得更细，一拍一短句）
- **切分粒度以「单行放得下」为准**：按句末标点 `。！？；…\n` 切，**逗号「，」「、」处也要拆**，使每 beat ≈ **4~12 个汉字**（约 1~3 秒配音）。**宁短勿长**——超过约 12~14 字必拆（FitLine 兜底缩字号，但别指望它，切句先保证短）。
- 短句**不再并入相邻句**（旧规则废除）：短就短着，一屏一句、节奏干净。
- 产出有序 `beats[]`，每个分配稳定 id：`p01, p02, …`，并标 `sceneId`（决定共图分组，见第 2 步）。
- **★ 金句原文 → 按上句/下句/分句逐拍朗读**（2026-07-03 定，2026-07-07 细化）：古诗/对句的**上句一拍、下句一拍**（更长的按分句继续拆），每拍分开读、分开上字幕；但**整句诗按一个场景共用 1 张图**（也允许按内容拆图，如「车马致千里」「舟楫绝江河」意象不同就 2 张）。
  - **★ 朗读画面的孩子**：出哪个孩子、出几张图**按该条文案与场景设计**（旧「一男一女随机/轮换」规则 2026-07-07 用户取消，不存在此规则）。不许为凑人数硬上多角色同框浪费 banana 名额。**配音永远统一旁白（开朗姐姐），不是孩子配音。**
- **★★ 文案以用户提供为准（2026-07-07 用户锁定，取代旧「固定引子/转换句」规则）**：
  - 旧规则「古语有云，」「这句话想告诉我们，」等**固定引子/转换句全部取消**，任何视频**不再插入任何固定句式/套话**（`config/prompts/storyboard.md` 旧铁律 15 作废，做新视频时同步删）。
  - 旁白/字幕**一切以用户交来的文案为准**：用户给什么念什么；需要扩写时（用户只给一句金句）按 [[00-overview]] 固定内容结构扩，但行文完全按**该条文案与场景**设计，不得套模板句。

### 第 2 步 · 分镜（LLM，固定 system prompt）
对每个 beat 由 LLM 判定并产出字段（写入 `script.json`）：
- `sceneId`：所属场景。**★ 2026-07-07 起 sceneId 即共图分组**：同 `sceneId` 的连续 beat **共用同一张图**（只出 1 次图），出图数 = 场景数 ≠ beat 数。切句变细后 beat 变多，**图别跟着变多**——一个语义画面（一句诗、一个演示情节）就一个 sceneId。
...
- 每个 beat 占时间窗 `[start, start + durationMs]`：
  - 播该 beat 配音；
  - 显该 beat **单行三语字幕**（一屏只有这一句短句）；
  - 画面显示该 beat 所属场景的图（**同 sceneId 连续 beat 不换图、不转场**，运镜跨拍连续；场景切换处才转场）。
- 中文字幕按字级时间戳 `charTimings[]` **逐字跳字**（已锁定，见 [[04-tts-captions]]、[[03-remotion-animation]]）。
